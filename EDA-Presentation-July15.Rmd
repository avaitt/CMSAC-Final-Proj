...
- To encapsulate the complex movement of the ball into a much simpler, useful object

- Given a location and other features, calculate expected Goals 
...
--
- Force vector field from a gravitational potential is conservative. 
- $F = MA$


--
- The negative gradient (downwards direction) of the Potential function gives the vector field of forces 
- $F = - \nabla V(x,y)$
...
- Gravity
$$ V(x,y) = -G\frac{1}{\sqrt{x^2 + y^2}} $$


--
- **NOT** "potential of scoring" => just underlying forces
...
- Take the Potential Functions of different teams (Offense and Defense), overlay offense against defense,
and then do random walks (steps sampled from poisson) to calculate probability of scoring.

<br>

- Detect Formation based on Potential Function Coefficients

<br>

- How the potential functions look for possessions leading to shots, success vs fails
